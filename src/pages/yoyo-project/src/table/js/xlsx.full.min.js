...
*/
    ...
    function _write(cfb, options) {
      ...
      var L = (function (cfb) {
        ...
        L[7] =
          (cfb.FileIndex[0].start = L[0] + L[1] + L[2] + L[3] + L[4] + L[5]) + ((L[6] + 7) >> 3);
        return L;
      })(cfb);
      var o = new_buf(L[7] << 9);
      var i = 0,
    ...
    function parse_zip(file, options) {
      ...
      var i = blob.length - 4;
      while (
        (blob[i] != 0x50 || blob[i + 1] != 0x4b || blob[i + 2] != 0x05 || blob[i + 3] != 0x06) &&
        i >= 0
      )
        --i;
      blob.l = i + 4;
  ...
  function parse_RkNumber(data) {
    ...
    b[0] &= 0xfc; // b[0] &= ~3;
    var RK =
      fInt === 0 ? __double([0, 0, 0, 0, b[0], b[1], b[2], b[3]], 0) : __readInt32LE(b, 0) >> 2;
  ...
  function parse_Lbl(blob, length, opts) {
    ...
    var rgce =
      target == blob.l || cce === 0 ? [] : parse_NameParsedFormula(blob, npflen, opts, cce);
    return {
      chKey: chKey,
      Name: name,
      itab: itab,
  ...
  var WK_ = (function () {
    ...
    function parse_NUMBER_17(blob, length) {
      ...
      o[1].v =
        (s * 2 - 1) *
        ((e > 0 ? v2 << e : v2 >>> -e) + (e > -32 ? v1 << (e + 32) : v1 >>> -(e + 32)));
      return o;
    }
...
*/
  ...
  function write_sty_xlml(wb, opts) {
    ...
    opts.cellXfs.forEach(function (xf, id) {
      var payload = [];
      payload.push(
        writextag("NumberFormat", null, { "ss:Format": escapexml(SSF._table[xf.numFmtId]) })
      );
...
*/
  ...
  function parse_dom_table(table, _opts) {
    ...
    ws["!ref"] = encode_range(range);
    if (R >= sheetRows)
      ws["!fullref"] = encode_range(((range.e.r = rows.length - _R + R - 1), range)); // We can count the real number of rows to parse but we don't to improve the performance
    return ws;
  }
